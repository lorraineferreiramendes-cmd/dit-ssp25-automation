...
def build_derived_sections(total_dit, backlog_d0, in_rota, para_desp, acionavel_otr, now):
            """Calcula secoes derivadas a partir dos dados principais do Tableau."""

    # STATUS LH: distribuicao estimada de Last Hour baseada nos status atuais
    lh = []
    if total_dit > 0:
                    lh_entregue = round(in_rota * 0.35)
                    lh_tentativa = round(in_rota * 0.12)
                    lh_ausente = round(in_rota * 0.03)
                    lh_total_check = lh_entregue + lh_tentativa + lh_ausente
                    lh = [
    ...
    # TIPO DE ENDERECO: distribuicao estimada por tipo
    endereco = []
    if total_dit > 0:
    ...
    # SORTING D0: distribuicao do sorting baseada no backlog
    sorting = []
    if total_dit > 0:
    ...
    # CAUSA RAIZ: principais causas de backlog
    causa_raiz = []
    if backlog_d0 > 0:
        ...
        if cr_outros < 0:
                        ...
                        causa_raiz = [
                                            {"l": "Pendente UTR",
                                                          "v": cr_utr, "p": round(cr_utr / backlog_d0 * 100, 1), "t": "bad"},
                                            {"l": "Cliente ausente",
                                                          "v": cr_ausente, "p": round(cr_ausente / backlog_d0 * 100, 1), "t": "warn"},
                                            {"l": "Endereco incorreto",
                                                          "v": cr_end_errado, "p": round(cr_end_errado / backlog_d0 * 100, 1), "t": "warn"},
                                            {"l": "Outros",
                                                          "v": cr_outros, "p": round(cr_outros / backlog_d0 * 100, 1), "t": "ok"},
                        ]

    # LOCALIZACAO ATUAL DIT
    localizacao = []
    if total_dit > 0:
                    loc_rota = in_rota
        loc_base = para_desp
        loc_utr = backlog_d0
        loc_transit = total_dit - loc_rota - loc_base - loc_utr
        if loc_transit < 0:
                            loc_transit = 0
                        localizacao = [
                                            {"l": "Em rota de entrega", "v": loc_rota,
                                                          "p": round(loc_rota / total_dit * 100, 1), "t": "ok"},
                                            {"l": "Na base / aguardando despacho", "v": loc_base,
                                                          "p": round(loc_base / total_dit * 100, 1), "t": "warn"},
                                            {"l": "Pendente UTR", "v": loc_utr,
                                                          "p": round(loc_utr / total_dit * 100, 1), "t": "bad"},
                                            {"l": "Em transito", "v": loc_transit,
                                                          "p": round(loc_transit / total_dit * 100, 1), "t": "ok"},
                        ]

    # CURVA DE EXPEDICAO: distribuicao por hora do dia (8h-18h)
    curva = []
    if total_dit > 0:
                    hora_atual = now.hour
        horas = list(range(8, 19))
        # Distribuicao normal centrada nas 10h-11h
